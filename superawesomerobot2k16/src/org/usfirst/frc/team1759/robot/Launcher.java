...
public class Launcher {
	...
	public double calcAngle(double camAngle, double camDistance){
		double outerLength = camDistance * (Math.cos(Math.toRadians(camAngle)));
		double innerLength = 12.75;
		double outerHeight = camDistance * (Math.sin(Math.toRadians(camAngle))); 
		double length = outerLength + innerLength;
		double newAng;
		if (length != 0){ 
			newAng = Math.toDegrees(Math.atan(outerHeight/length));
		}
		else{
			newAng = camAngle;
		}
	...
	public double calcDistance(double camAngle, double camDistance){
		double outerLength = camDistance * (Math.cos(Math.toRadians(camAngle)));
		double innerLength = 12.75;
		double outerHeight = camDistance * (Math.sin(Math.toRadians(camAngle))); 
		double length = outerLength + innerLength;
		double dist = Math.sqrt((Math.pow(length, 2))+Math.pow(outerHeight,2));
		return dist;
	}

	//papas vision angle: RIGHT NEGATIVE, LEFT POSITIVE
	public void autoTurn(double papasVisionAngle, double papasVisionDistance, double error){ //start error value at about 3 degrees
		double newAng = calcAngle(papasVisionAngle, papasVisionDistance);
		...
		//if papas vision angle is left of tower, need to move servo right, if papas vision angle is right of tower, need to move servo left
		double angle = Math.abs(newAng);
		double time = (angle/angRate);
		if (newAng > (0 + error)){ //left of tower, need to move servo right
			spinTalon.set(1.0);
			Timer.delay(time);
			spinTalon.set(0.0);
		}
		else if (newAng <(0 - error)){//right of tower, need to move servo left
			spinTalon.set(1.0);
			Timer.delay(time);
			spinTalon.set(0.0);
		}
